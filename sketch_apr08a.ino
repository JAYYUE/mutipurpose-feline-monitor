...
void setup() {
  ...
  //CAMERA
   //this is physical pin number 10.
   //give the RunCam2 10 seconds to stabilize
  digitalWrite(RunCamControl, LOW); //insure we start out with the control line low
  //attachInterrupt(digitalPinToInterrupt(cameraWifi), cameraShot, RISING);
}
...
void PushShutter() { //two pulses pushes the shutter to either take one picture or start video recording
  OnePulse();
  OnePulse();
